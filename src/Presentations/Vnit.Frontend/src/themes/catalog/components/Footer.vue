<template>
     <section class="footer">
        <div class="footer-container">
          <div class="t-footer">
            <div class="col-1">
              <a class="logo-footer" href="https://www.mioto.vn/">Mioto</a>
              <div class="space l"></div>
              <a class="func-social" href="https://www.facebook.com/mioto.vn/">
                <i class="ic ic-facebook"></i>
              </a>
              <a class="func-social" href="https://www.instagram.com/miotovn/">
                <i class="ic ic-instagram"></i>
              </a>
              <a class="func-social" href="https://twitter.com/miotovietnam">
                <i class="ic ic-twitter"></i>
              </a>
              <a class="func-social" href="https://plus.google.com/u/0/112340450598977299795">
                <i class="ic ic-google"></i>
              </a>
              <a class="func-social" href="https://www.linkedin.com/company/mioto/">
                <i class="ic ic-linkedin"></i>
              </a>
              <a class="func-social" href="https://www.youtube.com/channel/UCUbuFZt8RcIZUX4qC_BaV4Q">
                <i class="ic ic-youtube"></i>
              </a>
              <a class="func-social" href="https://www.pinterest.com/miotovietnam/">
                <i class="ic ic-pinterest"></i>
              </a>
            </div>
            <div class="col-2">
              <div class="f-part">
                <h5>Chính sách</h5>
                <ul>
                  <li>
                    <a href="https://www.mioto.vn/aboutus">Giới thiệu về Mioto</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/privacy">Chính sách và quy định</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/regu">Quy chế hoạt động</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/personalinfo">Bảo mật thông tin</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/resolveconflic">Giải quyết tranh chấp</a>
                  </li>
                </ul>
              </div>
              <div class="f-part">
                <h5>Tìm hiểu thêm</h5>
                <ul>
                  <li>
                    <a href="https://www.mioto.vn/howitwork">Hướng dẫn</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/terms">Nguyên tắc chung</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/ownerbenef">Trở thành chủ xe</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/faqs">Hỏi và trả lời</a>
                  </li>
                  <li>
                    <a href="https://www.mioto.vn/blogs">Mioto blog</a>
                  </li>
                </ul>
              </div>
              <div class="clear"></div>
            </div>
          </div>
          <div class="module-payment">
            <div class="payment-wrapper">
              <div class="left">
                <h4 class="title">CÁCH THỨC THANH TOÁN</h4>
                <div class="logo-payment">
                  <div class="payment-wrap">
                    <div class="fix-img">
                      <img src="https://www.mioto.vn/static/media/vtc.29280a18.png" alt="Mioto - Thuê xe tự lái">
                    </div>
                  </div>
                  <div class="payment-wrap">
                    <div class="fix-img">
                      <img src="https://www.mioto.vn/static/media/mastercard-logo.f6537046.png"
                        alt="Mioto - Thuê xe tự lái">
                    </div>
                  </div>
                  <div class="payment-wrap">
                    <div class="fix-img">
                      <img src="data:image/png;base64,"
                        alt="Mioto - Thuê xe tự lái">
                    </div>
                  </div>
                </div>
              </div>
              <div class="center">
                <h4 class="title">ĐỐI TÁC</h4>
                <div class="insurance-wrapper">
                  <div class="logo-insurance">
                    <a href="https://www.pjico.com.vn/" target="_blank" rel="noopener noreferrer">
                      <div class="fix-img">
                        <img src="https://www.mioto.vn/static/media/pjico.398ce6c5.png" alt="Mioto - Thuê xe tự lái">
                      </div>
                    </a>
                  </div>
                  <div class="logo-liberty">
                    <a href="https://www.libertyinsurance.com.vn/Direct/trang-chu/?utm_source=googlebrand" target="_blank"
                      rel="noopener noreferrer">
                      <div class=" fix-img">
                        <img src="https://www.mioto.vn/static/media/liberty-logo.3e57c587.png"
                          alt="Mioto - Thuê xe tự lái">
                      </div>
                    </a>
                  </div>
                </div>
              </div>
              <div class="right">
                <h4 class="title">CHỨNG NHẬN
                  <div class="certify-logo">
                    <div class="fix-img">
                      <a href="http://online.gov.vn/HomePage/WebsiteDisplay.aspx?DocId=41067" target="_blank" rel="noopener noreferrer">
                        <img src="https://www.mioto.vn/static/media/bo-cong-thuong.dd6d6139.png"
                          alt="Mioto - Thuê xe tự lái">
                      </a>
                    </div>
                  </div>
                </h4>
              </div>
            </div>
          </div>
          <div class="cpr">
            <div class="col-1">
              <span class="site">© Công ty Cổ Phần Mioto Việt Nam</span>
            </div>
            <div class="col-2">
              <ul class="nav-footer">
                <li>
                  <a>Địa chỉ: 305/4 Lê Văn Sỹ, Phường 1, Quận Tân Bình, Thành phố Hồ Chí Minh.</a>
                </li>
                <li>
                  <a>Mã số thuế: 0314714661</a>
                </li>
                <li>
                  <a>Email: contact@mioto.vn</a>
                </li>
                <li>
                  <a href="tel:19009217">Điện thoại: 1900 9217</a>
                </li>
                <li>
                  <a>Tên TK: Công ty Cổ phần Mioto Việt Nam </a>
                </li>
                <li>
                  <a>Số TK: 31310000905628 </a>
                </li>
                <li>
                  <a>Ngân hàng TMCP Đầu tư và Phát Triển VN - CN Nam Kỳ Khởi Nghĩa</a>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </section>
</template>
<script>
export default {

}
</script>

