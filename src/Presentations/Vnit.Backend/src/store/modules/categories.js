...
const actions = {
  getCategories ({ commit }, {query, pagination}) {
    ...
    let uri
    if (pagination) {
      uri = resourceUri + "?page=" + pagination.page + "&count=" + pagination.rowsPerPage
    }
    else uri = resourceUri + "?page=" + state.pagination.page + "&count=" + state.pagination.rowsPerPage
    if (query) {
      uri +=  "&name=" + query
    }
