...
const actions = {
  ...
  getOrderById ({ commit }, id) {
    if (id) {
      api.getData(resourceUri + "/" + id + "?_expand=customer").then(
        res => {
          const order = getResponseData(res);
  ...
  getProductsByCategory ({ commit }, categoryId) {
    if (categoryId) {
      api.getData("products?_expand=category&categoryId=" + categoryId).then(res => {
          const products = getResponseData(res).map(p => {
  ...
  getAllOrders ({ commit }) {
    ...
    api.getData(resourceUri + "?_expand=customer").then(res => {
      const orders = getResponseData(res);

      orders.forEach(item => {
        item.amount = item.orderItems.reduce(( p, c ) => p + ((c && c.unitPrice) || 0), 0);
        item.quantity = item.orderItems.length;
  ...
  searchOrders ({ commit }, searchQuery, pagination) {
    api.getData(resourceUri + "?" + searchQuery).then(res => {
      const orders = getResponseData(res);
      orders.forEach(item => {
        item.amount = item.orderItems.reduce((p, c) => p + c.unitPrice, 0);
        item.quantity = item.orderItems.length;
        item.customer = item.customer ? item.customer.firstName + " " + item.customer.lastName : "";
      });
      commitPagination(commit, orders);
  ...
  quickSearch ({ commit }, { headers, qsFilter, pagination }) {
    ...
    api.getData(resourceUri + "?name=" + qsFilter).then(res => {
      const orders = getResponseData(res).filter(r => headers.some(header => {
