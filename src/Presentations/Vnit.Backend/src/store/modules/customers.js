...
const actions = {
  getOrders ({ commit }) {
    api.getData("orders").then(res => {
      const orderList = getResponseData(res);
  ...
  getCustomerById ({ commit }, id) {
    ...
    if (id) {
      api.getData(resourceUri + "/" + id).then(
        res => {
          const customer = getResponseData(res);
  ...
  getAllCustomers ({ commit }) {
    ...
    api.getData("customers?_embed=orders").then(res => {
      const customers = getResponseData(res);
  ...
  quickSearch ({ commit }, { headers, qsFilter, pagination }) {
    ...
    api.getData("customers" + "?name=" + qsFilter).then(res => {
      const customers = getResponseData(res).filter(r =>
        headers.some(header => {
