...
using Vnit.Services.Events;

namespace Vnit.Services.Customers
{
    public class CustomerService : BaseEntityService<Customer>, ICustomerService
    {
        public CustomerService(IDataRepository<Customer> dataRepository) : base(dataRepository)
        {
        ...
        public virtual void DeleteCustomer(Customer customer)
        {
            ...
            if (customer.IsSystemAccount)
                throw new Exception($"System customer account ({customer.SystemName}) could not be deleted");
        ...
        public virtual Customer GetCustomerByUsername(string username)
        {
            ...
            var customer = base.FirstOrDefault(c => c.Username == username);
            return customer;
        }
